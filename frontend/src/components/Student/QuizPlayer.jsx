...
const QuizPlayer = () => {
  ...
  const getTotalSeconds = (quizData) => {
    ...
    const baseTimer = Number.isFinite(Number(quizData.timer)) && Number(quizData.timer) > 0 ? Number(quizData.timer) : 10;
    const questionCount = Array.isArray(quizData.questions) ? quizData.questions.length : 0;
    const minutes = quizData.timerPerQuestion
      ? baseTimer * questionCount
      : baseTimer;
    return minutes * 60;
  };

  const getTimerLabel = (quizData) => {
    if (!quizData) return '0 min total';
    const baseTimer = Number.isFinite(Number(quizData.timer)) && Number(quizData.timer) > 0 ? Number(quizData.timer) : 10;
    if (quizData.timerPerQuestion) {
      const totalMinutes = baseTimer * (quizData.questions?.length || 0);
      return `${baseTimer} min/question (${totalMinutes} min total)`;
    }
    return `${baseTimer} min total`;
  };
      ...
      try {
        ...
        if (backupData) {
          ...
          if (restore) {
            ...
            const restoredTime = Number.isFinite(backupData.timeRemaining) ? backupData.timeRemaining : totalSeconds;
            setTimeRemaining(Math.max(0, restoredTime));
      ...
      <div className="container py-4">
        {!quizStarted && (
          <div className="card card-glass p-4 text-center">
            ...
            <p>{quiz.questions.length} questions • {getTimerLabel(quiz)}</p>
